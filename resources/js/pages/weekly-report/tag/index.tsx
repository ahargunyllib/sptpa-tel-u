...
import type { DataWithPagination, Tag } from "@/types";
		...
		<DashboardLayout>
			...
			<section className="rounded-lg bg-white p-6 flex flex-col gap-4">
				<div className="flex flex-col gap-2">
					<h2 className="font-semibold text-base">Weekly Report Tag</h2>
						...
						<FormField
							...
							render={({ field }) => (
								<FormItem className="w-full">
									<FormControl>
										<Input placeholder="Tuliskan tag disini" {...field} />
									</FormControl>
									<FormMessage />
								</FormItem>
							)}
				...
				<div className="overflow-x-auto mt-4">
					<Table>
						<TableHeader>
							<TableRow>
								<TableHead className="text-xs text-[#667085]">No</TableHead>
						...
						<TableBody>
							{tags.data.map((tag) => (
								<TableRow key={tag.id}>
									...
									<TableCell className="text-sm font-medium">
										{new Date(tag.updated_at).toLocaleDateString("id-ID", {
											day: "2-digit",
											month: "2-digit",
											year: "numeric",
										})}
									...
									<TableCell className="w-[70px] flex gap-2">
										...
										<ConfirmationModal
											...
											trigger={<Trash className="w-4 h-4 text-danger-80" />}
										/>
									</TableCell>
								</TableRow>
							))}
